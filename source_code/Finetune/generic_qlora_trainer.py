...
class CustomControlCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state, control, **kwargs):
        ...
        pct_dataset = (records_processed / float(self.total_dataset_records)) * 100.0 if self.total_dataset_records > 0 else 0.0
        ...
        if now - self.last_heartbeat_time >= 60.0:
            ...
            print(f"[Heartbeat {time.strftime('%H:%M:%S')}] Step {state.global_step}/{state.max_steps:,} ({pct_run:.1f}% Run) | Processed: {records_processed:,} records | Loss: {loss_val:.4f} | Pace: {sec_per_it:.1f}s/it | Elapsed: {time_str}{vram_str} | Status: RUNNING OK")
...
def run_qlora_training(
    ...
    lr_decimal_str = f"{learning_rate:.6f}".rstrip('0').rstrip('.')
    ...
    print(f"[QLoRA] Config Profile: Batch Size = {batch_size} | Learning Rate = {lr_decimal_str} | Sub-sample = {sub_sample:,} | Max Steps = {max_steps} | Max Length = {max_length}")
    ...
    try:
        ...
        print("[QLoRA] Cleaning up GPU VRAM and system memory cache...")
